...
Count.averagePatient = (result) => {
  return dbConn.query("select ROUND(AVG(patients.age), 0) as avg_age, ROUND(AVG(patients.weight), 0) as avg_weight, ROUND(AVG(patients.height), 0) as avg_height " +
  "from conditions " +
  "left join patients_take_drugs using (condition_id) " +
  "left join patients using (patient_id) " +
  "where conditions.name = 'fever' " +
  "group by condition_id", (err, res) => {
    if (err) {
